...
class Rouge:
    ...
    def _get_scores(self, hyps, refs):
        ...
        for hyp, ref in zip(hyps, refs):
            ...
            for m in self.metrics:
                if m == "rouge-l":
                    _hyp = [" ".join(_.split()) for _ in hyp.split("。") if len(_) > 0]
                    _ref = [" ".join(_.split()) for _ in ref.split("。") if len(_) > 0]
                else:
                    _hyp = [" ".join(hyp.split())]
                    _ref = [" ".join(ref.split())]  
            ...
            if self.return_lengths:
                lengths = {
                    "hyp": len(" ".join(_hyp).split()),
                    "ref": len(" ".join(_ref).split())
                }
        ...
        for (hyp, ref) in zip(hyps, refs):
            for m in self.metrics:
                if m == "rouge-l":
                    ...
                    _ref = [" ".join(ref.split())]         
                fn = Rouge.AVAILABLE_METRICS[m]
                sc = fn(_hyp, _ref, exclusive=self.exclusive)
                scores[m] = {s: scores[m][s] + sc[s] for s in self.stats}


            if self.return_lengths:
                scores["lengths"]["hyp"] += len(" ".join(_hyp).split())
                scores["lengths"]["ref"] += len(" ".join(_ref).split())
